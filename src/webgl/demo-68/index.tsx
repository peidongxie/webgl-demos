...
type DemoState = StateWithRoot<{
  ...
  u_Eye: WebGLUniformLocation | null;
 ...
 * 线性雾化
 */
const Demo68: FC<ComponentProps> = () => {
    ...
    (canvas: HTMLCanvasElement, gl: WebGLRenderingContext) => {
      const draw = makeWebGLDraw<DemoState>(
        ...
        (program) => ({
          ...
          u_Eye: {
            deps: ['camera'],
            data: gl.getUniformLocation(program!, 'u_Eye'),
            onChange: ({ u_Eye, camera }) => {
              const [eyeX, eyeY, eyeZ] = camera;
              gl.uniform4f(u_Eye, eyeX, eyeY, eyeZ, 1);
            },
          },
          // 着色器变量：u_FogColor
          u_FogColor: {
